...
module.exports = {
    parsePost : async (query) => {
        ...
        let postings = result.data.items.map(arg => {
            ...
            return ({id: 'n', title: arg.title.replace(/<\/?[^>]+(>|$)/g, "").replace(/&lt;/g,'').replace(/&gt;/g,'').replace(/&amp;/g,'&').replace(/&quot;/g, '"'), link: arg.link, contents: arg.description.replace(/<\/?[^>]+(>|$)/g, "").replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&amp;/g,'&').replace(/&quot;/g, '\\"').substring(0, 30)+'...'})
        });
        return postings;
    },
